...
class Data:
    ...
    def get_underlying_details(self, underlying_symbols: list[str]):
        for ticker in underlying_symbols:
            ...
            self.dividend_yield_dict[ticker] = ticker.info["dividendYield"] / 100
            self.asset_price_dict[ticker] = ticker.info["previousClose"]
    ...
    def get_active_options_api(self, underlying_symbols: list[str], limit: int = 1000):
        for ticker in underlying_symbols:
            ...
            self.contracts_dict[ticker].to_csv(f"{ticker}_options.csv")
    ...
    def clean_up_df(self, df: pd.DataFrame):
        ...
        df["days_to_expiry"] = df["expiration_date"] - datetime.now()
